...
/* Integridad refencial 
	Cuando se define una columna como clave foránea , las filas de la tabla pueden contener n esa columna o bien el valor nulo(ningun valor), o bien un valor que existe en la otra tabla , un error sería asignar a un habitante una poblacion que no esta en la tabla de operaciones . Eso es lo que se denomina INTEGRIDAD REFERENCIAL y consites en que LOS DATOS QUE REFERENCIAN OTROS (CLAVES FORÁNEAS) DEBEN SER CONCRETOS . La INTEGRIDAD REFERENCIAL hace que el sistema gestor de base de datos se asegure de que no hayan las claves foráneas valores que no estén en la tabla principal

	La integridad refencial se activa cuando creamos una clave foránea y a partir de ese momento se comprueba cada vez que se modifquen datos que puedan alterarla
